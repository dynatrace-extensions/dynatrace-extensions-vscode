 ...
 */
export async function uploadCertificate(context: vscode.ExtensionContext, dt: Dynatrace): Promise<boolean> {
  var certFile = readdirSync(path.join(context.storageUri!.fsPath, "certificates")).find((f) => f === "ca.pem");
  if (!certFile) {
  ...
  var pemCert = readFileSync(path.join(context.storageUri!.fsPath, "certificates", "ca.pem")).toString();
  ...
  if (update) {
    ...
    if (!certName || certName === "") {
      vscode.window.showErrorMessage("Certificate name cannot be blank. Operation was cancelled.");
      return false;
    }
